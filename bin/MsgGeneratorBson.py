...
def GenFromBson(Variables):
    FromBson = []
    FromBson.append('virtual void FromBson(TSharedPtr<FBsonObject> BsonObject) override\n')
    FromBson.append('{\n')
    ArrayFlag = False
    ...
    for Variable in Variables:
        if(not Variable.IsArray()):
            if(Variable.GetJsonType() == 'ObjectField'):
                FromBson.append('\t' + Variable.GetName() + ' = ' + Variable.GetType() + '::GetFromBson(BsonObject->GetObjectField(TEXT("' + Variable.GetOriginalName() + '")));\n\n')
            else:
                FromBson.append('\t' + Variable.GetName() + ' = BsonObject->Get' + Variable.GetJsonType() + '(TEXT("' + Variable.GetOriginalName() + '"));\n\n')
        else:
            if(not ArrayFlag):
                FromBson.append('\t' + 'TArray<TSharedPtr<FBsonValue>> ValuesPtrArr;\n\n')
                ArrayFlag = True
            FromBson.append('\t' + Variable.GetName() + '.Empty();\n')
            FromBson.append('\t' + 'ValuesPtrArr = BsonObject->GetArrayField(TEXT("' + Variable.GetOriginalName() + '"));\n')
            FromBson.append('\t' + 'for (auto &ptr : ValuesPtrArr)\n')
            if(Variable.GetJsonType() == 'ObjectField'):
                FromBson.append('\t' + '\t' + Variable.GetName() + '.Add(' + Variable.GetType() + '::GetFromBson(ptr->AsObject()));\n\n')
            else:
                FromBson.append('\t' + '\t' + Variable.GetName() + '.Add(ptr->As' + Variable.GetJsonType()[:-5] + '());\n\n')

    FromBson.append('}\n\n')

    return Indent(FromBson, 2)
...
def GenGetFromBson(ClassName):
    GetFromBson = []
    GetFromBson.append('static ' + ClassName + ' GetFromBson(TSharedPtr<FBsonObject> BsonObject)\n')
    GetFromBson.append('{\n')
    GetFromBson.append('\t'+ ClassName + ' Result;\n')
    GetFromBson.append('\t'+ 'Result.FromBson(BsonObject);\n')
    GetFromBson.append('\t'+ 'return Result;\n')
    GetFromBson.append('}\n\n')

    return Indent(GetFromBson, 2)
...
def GenToBsonObject(Variables):
    ToBsonObject = []
    ToBsonObject.append('virtual TSharedPtr<FBsonObject> ToBsonObject() const override\n')
    ToBsonObject.append('{\n')
    ToBsonObject.append('\tTSharedPtr<FBsonObject> Object = MakeShareable<FBsonObject>(new FBsonObject());\n\n')

    for Variable in Variables:
        if(Variable.IsArray()):
            ToBsonObject.append('\tTArray<TSharedPtr<FBsonValue>> ' + Variable.GetName() + 'Array;\n')
            ToBsonObject.append('\tfor (auto &val : ' + Variable.GetName() + ')\n')
            if(Variable.GetJsonType() == 'ObjectField'):
                ToBsonObject.append('\t\t' + Variable.GetName() + 'Array.Add(MakeShareable(new FBsonValue' + Variable.GetJsonType()[:-5] + '(val.ToBsonObject())));\n')
            else:
                ToBsonObject.append('\t\t' + Variable.GetName() + 'Array.Add(MakeShareable(new FBsonValue' + Variable.GetJsonType()[:-5] + '(val)));\n')
            ToBsonObject.append('\tObject->SetArrayField(TEXT("' + Variable.GetOriginalName() + '"), ' + Variable.GetName() + 'Array);\n' )
        else:
            if(Variable.GetJsonType() == 'ObjectField'):
                ToBsonObject.append('\tObject->SetObjectField(TEXT("' + Variable.GetOriginalName() + '"), ' + Variable.GetName() + '.ToBsonObject());\n')
            else:
                ToBsonObject.append('\tObject->Set' + Variable.GetJsonType() + '(TEXT("' + Variable.GetOriginalName() + '"), ' + Variable.GetName() + ');\n')

    ToBsonObject.append('\treturn Object;\n')
    ToBsonObject.append('}\n')
    return Indent(ToBsonObject, 2)
    ...
    if(Package.exists()):
        for file in list(Package.glob('*.msg')):
            with file.open() as MsgFile:
                ...
                OutputArray.append(GenFromBson(Variables))
                OutputArray.append(GenGetFromJson(MsgName))
                OutputArray.append(GenGetFromBson(MsgName))
                OutputArray.append(GenToJsonObject(Variables))
                OutputArray.append(GenToBsonObject(Variables))
