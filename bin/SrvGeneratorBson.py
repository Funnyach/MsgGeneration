    ...
    for Variable in Variables:
        if(not Variable.IsArray()):
            ...
            if(Variable.GetJsonType() == 'ObjectField'):
                ...
                FromBson += '\t' + Variable.GetName() + '.Add(ptr->As' + Variable.GetJsonType()[:-5] + '());\n\n' + '\t' * Indent
...
def GenToBsonObject(Variables):
    ...
    for Variable in Variables:
        if(Variable.IsArray()):
            ...
            if(Variable.GetJsonType() == 'ObjectField'):
                ToBsonObject += '\t' + Variable.GetName() + 'Array.Add(MakeShareable(new FBsonValue' + Variable.GetJsonType()[:-5] + '(val.ToBsonObject())));\n'+ '\t' * Indent
            else:
                ToBsonObject += '\t' + Variable.GetName() + 'Array.Add(MakeShareable(new FBsonValue' + Variable.GetJsonType()[:-5] + '(val)));\n'+ '\t' * Indent
